...
reduc_resp.ind 	= which(colnames(input) == resp.var)
...
##### Conduct CV for variable selection & performance evaluation
eval.result = list()	### Container for final results
CV.k = c(3,5)			### Vector of 'k' values to conduct CV with
 
	### Conduct k-fold CV per each 'k' in the prespecified 'CV.k'
for (k in CV.k) {
	result = list()
	...
	for (i in 1:k) {
		...
		orig_train.dat 	= input[-folds[[i]],]
		reduc_train.dat = reduc_input[-folds[[i]],]
		test.dat 		= input[folds[[i]],]
		
		### Variable selection
			### Function call for stepwise AIC
		#result_AIC	= stepwiseAIC(reduc_train.dat,resp.var)
		...
		reg.k 		= 5				### 'k' value for CV for determining 'lambda' for regularization
		result_auc	= regular.CV(orig_train.dat, orig_resp.ind, reg.k, thres=eps, crit="auc");   result_auc
		result_dev	= regular.CV(orig_train.dat, orig_resp.ind, reg.k, thres=eps, crit="dev");   result_dev
		result_cls	= regular.CV(orig_train.dat, orig_resp.ind, reg.k, thres=eps, crit="class"); result_cls
		result_mae	= regular.CV(orig_train.dat, orig_resp.ind, reg.k, thres=eps, crit="mae");   result_mae
		
		### 'marker.mat' generation (store variable selection results in binary format)
			### Conversion of AIC variable selection results to original variable set
		AIC_0 = ifelse(exp.vars %in% colnames(reduc_train.dat)[-ncol(reduc_train.dat)][result_AIC$step0 == 1],1,0); AIC_0
		AIC_F = ifelse(exp.vars %in% colnames(reduc_train.dat)[-ncol(reduc_train.dat)][result_AIC$stepF == 1],1,0); AIC_F
		...
		# sort(apply(marker.mat,2,sum),decreasing=T)
 		...
 		result[[i]]	= performance(test.dat,orig_resp.ind,marker.mat)			### TODO fill in arguments
	}
	
		### Compute mean of CV results
	result[[paste0("MEAN_",k)]] = Reduce("+",result) / k
	
		### Store computation results to container
	eval.result[[k]] = result
}
